...
namespace Denismitr\LaraRabbit\Horizon;
...
use Denismitr\LaraRabbit\Queue\RabbitMQQueue as BaseRabbitMQQueue;
    ...
    public function pop($queue = null)
    {
        return tap(parent::pop($queue), function ($result) use ($queue): void {
            if ($result instanceof RabbitMQJob) {
                $this->event($this->getQueueName($queue), new JobReserved($result->getRawBody()));
            }
        });
    }
